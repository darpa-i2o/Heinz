...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=1000, embed_dim = 10, hidden_dim = 10):
        ...
        self.avg_pool_three = nn.AvgPool2d(kernel_size=2, stride=2)
        ...
        for i in range(block_config[0]):
            ...
            gate_layer = nn.Sequential(
                nn.AvgPool2d(56),
                nn.Conv2d(in_channels = num_features + (i + 1) * growth_rate,
            ...
            if input_channels != growth_rate:
                downsample = nn.Sequential(
                    nn.Conv2d(input_channels,growth_rate,
                              kernel_size = 1, stride = 1, bias = False),
        ...
        for i in range(block_config[3]):
            ...
            setattr(self, 'denseblock3_%s_gate' % i, gate_layer)
            ...
            if input_channels != growth_rate:
                ...
                setattr(self, 'denseblock3_%s_ds' % i, downsample)
    ...
    def forward(self, x):
        ...
        for i in range(self.block_config[3]):
            ...
            if i == 0:
                prev_new_features = self.avg_pool_three(prev_new_features)
            ...
            gate_feature = getattr(self, 'denseblock3_{}_gate'.format(i))(features)
            mask, gprob = self.control(gate_feature)
