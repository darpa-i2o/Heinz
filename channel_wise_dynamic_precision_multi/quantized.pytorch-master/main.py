...
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None):
    ...
    for layers in range(3):
        for block in range(model.layers[0]):
            channels = getattr(model,'layer{}'.format(layers + 1))[block].planes
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        for layer in range(1 + 2 * num_layers):
            ...
            computation_costs[0] += reduce((lambda x, y: x * y), Masks[layer][0].shape) * computation_parameters[0][layer]
            
            full_layer = reduce((lambda x, y: x * y), Masks[layer][0].shape)
            
            for k in range(1,len(bits)):
                computation_costs[k] += Masks[layer][k - 1].sum() * computation_parameters[k][layer]
        ...
        if i % args.print_freq == 0:
            logging.info('{phase} - Epoch: [{0}][{1}/{2}]\t'
                         ...
                         'Compression_rate: {cp_record.val:.3f}({cp_record.avg:.3f})\t'.format(
    ...
    for layer in range(num_convs):
        ...
        for g in range(1,len(bits)):
            bit = bits[g]
            print('{}bit_ratio{}'.format(bit,bit_assignment_statistics[layer][g - 1].avg))
