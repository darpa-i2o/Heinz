...
import logging
import models_layer_skip_initial
import numpy as np
from functools import reduce

model_names = sorted(name for name in models_layer_skip_initial.__dict__
                     if name.islower() and not name.startswith("__")
                     and callable(models_layer_skip_initial.__dict__[name]))
...
def parse_args():
    ...
    parser.add_argument('--gate_type', default='rnn',
                        help='Whether rnn gate is applied')
...
def main():
    args = parse_args()
    args.save_path = save_path = os.path.join(args.save_folder, args.arch + '_skip_layer_minimum_' + str(args.minimum))
...
def run_training(args):
    # create model
    model = models_layer_skip_initial.__dict__[args.arch](args.pretrained)
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    # for i in range(4):
        # for j in range(model.module.block_config[i]):
            ...
            # cost = (1/4) ** i * (1 + (num_input_feature / growth_rate) * (1/9))
